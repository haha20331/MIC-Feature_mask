...
#############################調整threshold大小###################################
    ...
    def cal_loss(self,logits,pseudo_label,seg_weight,name):
        losses_dict = dict()
        losses = self.losses(logits, pseudo_label, seg_weight)
        losses_dict.update(add_prefix(losses, 'decode'))
        losses = add_prefix(losses_dict, name)
        loss,var = self._parse_losses(losses)
...
############## get model是拿hrda_encoder_decoder #############
            ...
            if self.student_consistency_loss_flag and self.student_mix_img_loss_flag:
                mix_loss = 0.5*(student_mix_loss + mix_loss)
...
##### weak aug 的feature mask #####
            if self.mask_feature_ratio['flag']:
                ...
                log_vars.update(mask_feature_log_vars)
                if self.student_consistency_loss_flag and self.student_mask_feature_loss_flag:
                    mask_feature_loss = 0.5*(student_mask_feature_loss + mask_feature_loss)
                    del student_mask_feature_loss
                mask_feature_loss.backward()
#############################################################
...
############## Masked Image Training ##############
        if self.enable_masking and self.mask_mode.startswith('separate'):
            ...
            if self.student_consistency_loss_flag and self.student_mask_img_loss_flag:
                masked_loss = 0.5*(student_mask_image_loss + masked_loss)
        ...
        if self.local_iter % self.debug_img_interval == 0 and \
            ...
            for j in range(1):
                ...
                if self.student_consistency_loss_flag:    
                    ...
                    subplotimg(
                        axs[2][3], student_mix_pseudo_label[j], 'Student A1 + MixGT',cmap='cityscapes')
                    subplotimg(
