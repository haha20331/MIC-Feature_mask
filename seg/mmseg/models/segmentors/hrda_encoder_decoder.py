...
class HRDAEncoderDecoder(EncoderDecoder):
    ...
    def extract_unscaled_feat(self, img, mask_feature_ratio=dict(flag=False)):
        ################試著多傳一個參數進去backbone##################
        x = self.backbone(img, mask_feature_ratio)
        if self.with_neck:
            x = self.neck(x)
        return x
    ...
    def _forward_train_features(self, img, mask_feature_ratio=dict(flag=False)):
        mres_feats = []
        self.decode_head.debug_output = {}
        ...
        for i, s in enumerate(self.scales):
            ...
            mres_feats.append(self.extract_unscaled_feat(scaled_img, mask_feature_ratio))
    ...
    def forward_train(self,
        ...
        losses = dict()
########## 是F1~F4 + scale ###################
        mres_feats, prob_vis = self._forward_train_features(img, mask_feature_ratio)
